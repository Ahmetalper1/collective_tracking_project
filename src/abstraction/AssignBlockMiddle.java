...
public class AssignBlockMiddle
{
			...
			MiddleAutomaton middleAutomaton)
	{
		...
		for (; temp_i <= i + 1; temp_i++)
		{
			for (; temp_j <= j + 1; temp_j++)
			{
				if ((temp_i != i || temp_j != j)
					&& CheckCoords
						.verifyCoordinatesMiddle(temp_i,
						temp_j))
				{
					if (middleCells[temp_i][temp_j].getBlock() != -1)
					{
						middleCells[i][j]
								.setBlock(middleCells[temp_i][temp_j]
										.getBlock(), false);
						Game.middleBlocks.get(middleCells[temp_i][temp_j]
								.getBlock()).addCell(middleCells[i][j]);
						return;
					}
				}
			}
		...
		for (; temp_i <= i + 1; temp_i++)
		{
			for (; temp_j <= j + 1; temp_j++)
			{
						...
						temp_j))
				{
					...
					if (middleCells[temp_i][temp_j].getPreviousBlock() != -1)
					{
						middleCells[i][j]
								.setBlock(middleCells[temp_i][temp_j]
										.getPreviousBlock(), false);
						Game.middleBlocks.get(middleCells[temp_i][temp_j]
								.getPreviousBlock()).addCell(middleCells[i][j]);
						return;
					}
				}
			}
		 ...
		 */
		middleCells[i][j].setBlock(BlockDetector.blockIDMiddle, false);
