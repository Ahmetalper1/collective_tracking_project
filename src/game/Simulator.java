...
public class Simulator extends Thread
{
	...
	public Simulator(Game game)
	{
		...
		this.bottomGraph = new Graph(game, AutomatonLevel.BOTTOM,
				Graph.DEFAULT_BOTTOM_ID);
    ...
    public void run() {
		...
		while (true)
		{
			try
			{
				 ...
				 *  Also updating the other automata after the iteration on the
				 *  bottom one is finished.
				 */
			...
			{
				LoggerInstance.LOGGER.log(Level.FINEST, "InterruptedException"
			...
			catch (IOException e)
			{
				LoggerInstance.LOGGER.log(Level.SEVERE, "IOException in "
						+ "Simulator.run(): Could not write the output files."
	...
	private void setWindowsLocations()
	{
		...
		game.getBottomAutomaton().getGrid()
			.display((screen.width / 2) - (View.BOTTOM_WIDTH / 2),
		...
		int heightOffset = screen.height - View.BOTTOM_HEIGHT
				- View.MIDDLE_HEIGHT;
		...
		for (MiddleAutomaton middle : game.getMiddleAutomaton())
		{
			middle.getGrid().display(widthOffset - (View.MIDDLE_WIDTH / 2),
					heightOffset, "Middle Automaton | "
							+ Game.MIDDLE_ROW + "x"
							+ Game.MIDDLE_COLUMN
							+ " | Threshold : " + middle.getThreshold() + ".");

			widthOffset += screen.width
					/ (game.getMiddleAutomaton().size() + 1);
		}

		// Updating the offsets for the top automata.
		widthOffset = screen.width / (game.getTopAutomaton().size() + 1);
		heightOffset = screen.height - View.BOTTOM_HEIGHT - View.MIDDLE_HEIGHT
			- View.TOP_HEIGHT;
	...
	private void createMiddleGraphs()
	{
		...
		{
			this.middleGraphs.add(
					new Graph(this.game, AutomatonLevel.MIDDLE, i));
		}
	}
}
